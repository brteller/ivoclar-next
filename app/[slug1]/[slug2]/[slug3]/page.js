...
export async function generateMetadata({ params }) {
  ...
  const paramsNorm = { country: p.slug1, state: p.slug2, category: p.slug3 };
  const headersList = await headers();
  const origin = getOrigin(headersList);
  let data = null;
  try {
    data = await fetchPageData({ category: paramsNorm.category, country: paramsNorm.country, state: paramsNorm.state });
  } catch (e) {
    console.error(e);
  }
...
export default async function StateCategoryPage({ params }) {
  ...
  const pathname = `/${p.slug1}/${p.slug2}/${p.slug3}`;
  return (
    <PageRenderer
      htmlContent={data}
      pathname={pathname}
